...
const NotesSummarizer = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL.replace(/\/$/, '')}/api/pdf-extract`, {
        method: 'POST',
        body: formData
      });
  ...
  const handleSummarize = async () => {
    ...
    setSummary("");
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL.replace(/\/$/, '')}/api/chat`, {
        method: "POST",
